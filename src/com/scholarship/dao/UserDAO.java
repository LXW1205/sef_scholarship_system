    ...
    public User authenticate(String username, String password) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    return fetchDetailedUser(rs.getInt("userID"), rs.getString("username"), rs.getString("email"),
            ...
            case "Student":
                ...
                try (PreparedStatement uStmt = conn.prepareStatement(studentSql)) {
                    ...
                    try (ResultSet uRs = uStmt.executeQuery()) {
                        if (uRs.next()) {
                            return new Student(userId, username, email, isActive, uRs.getString("studentID"),
                                    uRs.getString("fullName"), uRs.getDouble("cgpa"));
                        }
                    }
                }
                break;
            case "Reviewer":
                ...
                try (PreparedStatement rStmt = conn.prepareStatement(revSql)) {
                    ...
                    try (ResultSet rRs = rStmt.executeQuery()) {
                        if (rRs.next()) {
                            return new Reviewer(userId, username, email, isActive, rRs.getString("staffID"),
            ...
            case "CommitteeMember":
                ...
                try (PreparedStatement cmStmt = conn.prepareStatement(cmSql)) {
                    ...
                    try (ResultSet cmRs = cmStmt.executeQuery()) {
                        if (cmRs.next()) {
                            return new CommitteeMember(userId, username, email, isActive, cmRs.getInt("memberID"),
            ...
            case "Admin":
                ...
                try (PreparedStatement aStmt = conn.prepareStatement(adminSql)) {
                    ...
                    try (ResultSet aRs = aStmt.executeQuery()) {
                        if (aRs.next()) {
                            return new Admin(userId, username, email, isActive, aRs.getInt("adminID"),
