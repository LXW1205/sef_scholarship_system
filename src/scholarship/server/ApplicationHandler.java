...
import scholarship.dao.ApplicationDAO;
import scholarship.dao.EvaluationDAO;
...
public class ApplicationHandler implements HttpHandler {
    ...
    private scholarship.dao.NotificationDAO notificationDAO = new scholarship.dao.NotificationDAO();
    private scholarship.dao.UserDAO userDAO = new scholarship.dao.UserDAO();
    ...
    private void handleCreateApplication(HttpExchange exchange, String requestBody) throws IOException {
        ...
        try {
            ...
            scholarship.model.Scholarship scholarshipObj = sDAO.findById(scholarshipID);
            scholarship.model.Student student = (scholarship.model.Student) userDAO.findUserByRoleID(studentID,
                    "Student");

            if (scholarshipObj == null || student == null) {
                sendError(exchange, 404, "Scholarship or Student not found");
                return;
            }

            // Perform hard eligibility check
            if (!ScholarshipHandler.isEligible(student, scholarshipObj)) {
            ...
            if (requestBody.contains("\"documents\":")) {
                ...
                if (transcript != null)
                    app.addDocument(new scholarship.model.Document(0, 0, transcript, "Transcript",
                            transcriptContent, null));
                if (recommendation != null)
                    app.addDocument(new scholarship.model.Document(0, 0, recommendation, "Recommendation",
                            recommendationContent, null));
                if (idCard != null)
                    app.addDocument(new scholarship.model.Document(0, 0, idCard, "ID", idContent, null));
            }
            ...
            if (appId > 0) {
                ...
                // AUDIT LOG
                scholarship.dao.AuditLogDAO.log(null, studentID, "Application Submitted", "Application",
        ...
        try {
            ...
            if (appIdStr != null && reviewerId != null) {
                ...
                if (evaluationDAO.save(eval)) {
                    ...
                    // NOTIFY REVIEWER
                    scholarship.model.User reviewer = userDAO.findUserByRoleID(reviewerId, "Reviewer");
                    if (reviewer != null) {
                        notificationDAO.create(new scholarship.model.Notification(0, reviewer.getId(),
                                "You have been assigned a new application (ID: " + appId + ") for review.", null,
                                false));

                        // AUDIT LOG
                        scholarship.dao.AuditLogDAO.log(null, "Admin", "Reviewer Assigned", "Application",
            ...
            if (eval != null) {
                ...
                if (evaluationDAO.update(eval)) {
                    ...
                    if (scoresJson != null && scoresJson.startsWith("[")) {
                        List<scholarship.model.EvaluationScore> scoresList = new java.util.ArrayList<>();
                        ...
                        if (!clean.isEmpty()) {
                            ...
                            for (String obj : scoreObjs) {
                                ...
                                if (cIdStr != null && sStr != null) {
                                    ...
                                    scoresList.add(
                                            new scholarship.model.EvaluationScore(0, eval.getEvalID(), cId, sc));
                                }
                            }
                        }
                        System.out.println(
                    ...
                    // AUDIT LOG
                    scholarship.dao.AuditLogDAO.log(null, eval.getReviewerID(), "Evaluation Submitted",
                            "Application",
    ...
    private void handleDecision(HttpExchange exchange, String requestBody) throws IOException {
        ...
        try {
            ...
            if (applicationDAO.updateStatus(appId, decision, comments)) {
                ...
                // AUDIT LOG
                scholarship.dao.AuditLogDAO.log(null, "Admin", "Application " + decision, "Application",
    ...
    private void notifyStudentStatusChange(int appId, String newStatus, String details) {
        try {
            Application app = applicationDAO.findById(appId);
            if (app != null) {
                scholarship.model.User student = userDAO.findUserByRoleID(app.getStudentID(), "Student");
                if (student != null) {
                    ...
                    notificationDAO
                            .create(new scholarship.model.Notification(0, student.getId(), msg, null, false));
                }
            }
        } catch (Exception e) {
            ...
            if (appIdStr != null && studentId != null) {
                ...
                if (app != null && app.getStudentID().equals(studentId)) {
                    ...
                    if (applicationDAO.updateStatus(appId, StatusValidator.APP_WITHDRAWN)) {
                        ...
                        // AUDIT LOG
                        scholarship.dao.AuditLogDAO.log(null, studentId, "Application Withdrawn", "Application",
    ...
    private String formatApplicationsJson(List<Application> apps) {
        ...
        try {
            ...
            for (Application a : apps) {
                ...
                List<scholarship.model.Document> docs = a.getDocuments();
                for (int i = 0; i < docs.size(); i++) {
                    scholarship.model.Document d = docs.get(i);
                ...
                if (eval != null) {
                    ...
                    List<scholarship.model.EvaluationScore> scores = evaluationDAO
                            .findScoresByEvalId(eval.getEvalID());
                    for (int i = 0; i < scores.size(); i++) {
                        scholarship.model.EvaluationScore s = scores.get(i);
    ...
    private String getApplicationJsonById(int appId) {
        ...
        scholarship.dao.ScholarshipDAO sDAO = new scholarship.dao.ScholarshipDAO();
        scholarship.model.Scholarship sObj = sDAO.findById(a.getScholarshipID());

        StringBuilder json = new StringBuilder();
        json.append(formatApplicationJsonSingle(a, sObj));
        return json.toString();
    }

    // Helper to format a single application nicely including criteria
    private String formatApplicationJsonSingle(Application a, scholarship.model.Scholarship sObj) {
        String reviewerInfo = "";
        if (a.getReviewerID() != null) {
            reviewerInfo = String.format(", \"reviewerId\": \"%s\", \"reviewerName\": \"%s\"",
                    JsonUtils.escape(a.getReviewerID()), JsonUtils.escape(a.getReviewerName()));
        ...
        if (sObj != null) {
            boolean first = true;
            for (scholarship.model.Criterion c : sObj.getCriteria()) {
                if (!first)
                    criteriaJson.append(",");
                first = false;
        ...
        if (eval != null) {
            ...
            List<scholarship.model.EvaluationScore> scores = evaluationDAO.findScoresByEvalId(eval.getEvalID());
            for (int i = 0; i < scores.size(); i++) {
                scholarship.model.EvaluationScore sc = scores.get(i);
