...
public class InterviewHandler implements HttpHandler {
    ...
    private void handleCreate(HttpExchange exchange, String requestBody) throws IOException {
        try {
            ...
            if (interviewId > 0) {
                // Notify student
                Evaluation eval = evaluationDAO.findByAppId(getAppIdFromEvalId(evalId));
                if (eval != null) {
                    ...
                    if (app != null) {
                        ...
                        if (student != null) {
                            ...
                            notificationDAO.create(
                                    new scholarship.model.Notification(0, student.getId(), msg, null, false));
                        }
                    }
                }

                // Audit log
                scholarship.dao.AuditLogDAO.log(null, "Committee", "Interview Scheduled", "Interview",
    ...
    private Evaluation findEvalByEvalId(int evalId) {
        ...
        try {
            ...
            java.sql.Connection conn = scholarship.db.DatabaseConnection.getConnection();
