...
public class ApplicationHandler implements HttpHandler {
    ...
    private void handleCreateApplication(HttpExchange exchange, String requestBody) throws IOException {
        ...
        try {
            ...
            model.Scholarship scholarshipObj = sDAO.findById(scholarshipID);
            model.Student student = (model.Student) userDAO.findUserByRoleID(studentID,
                    "Student");
            ...
            if (requestBody.contains("\"documents\":")) {
                ...
                if (transcript != null)
                    app.addDocument(new model.Document(0, 0, transcript, "Transcript",
                            transcriptContent, null));
                if (recommendation != null)
                    app.addDocument(new model.Document(0, 0, recommendation, "Recommendation",
                            recommendationContent, null));
                if (idCard != null)
                    app.addDocument(new model.Document(0, 0, idCard, "ID", idContent, null));
            }
            ...
            if (appId > 0) {
                ...
                // AUDIT LOG
                dao.AuditLogDAO.log(null, studentID, "Application Submitted", "Application",
        ...
        try {
            ...
            if (appIdStr != null && reviewerId != null) {
                ...
                if (evaluationDAO.save(eval)) {
                    ...
                    // NOTIFY REVIEWER
                    model.User reviewer = userDAO.findUserByRoleID(reviewerId, "Reviewer");
                    if (reviewer != null) {
                        notificationDAO.create(new model.Notification(0, reviewer.getId(),
                                "You have been assigned a new application (ID: " + appId + ") for review.", null,
                                false));

                        // AUDIT LOG
                        dao.AuditLogDAO.log(null, "Admin", "Reviewer Assigned", "Application",
            ...
            if (eval != null) {
                ...
                if (evaluationDAO.update(eval)) {
                    ...
                    if (scoresJson != null && scoresJson.startsWith("[")) {
                        List<model.EvaluationScore> scoresList = new java.util.ArrayList<>();
                        ...
                        if (!clean.isEmpty()) {
                            ...
                            for (String obj : scoreObjs) {
                                ...
                                if (cIdStr != null && sStr != null) {
                                    ...
                                    scoresList.add(
                                            new model.EvaluationScore(0, eval.getEvalID(), cId, sc));
                                }
                            }
                        }
                        System.out.println(
                    ...
                    // AUDIT LOG
                    dao.AuditLogDAO.log(null, eval.getReviewerID(), "Evaluation Submitted",
                            "Application",
    ...
    private void handleDecision(HttpExchange exchange, String requestBody) throws IOException {
        ...
        try {
            ...
            if (applicationDAO.updateStatus(appId, decision, comments)) {
                ...
                // AUDIT LOG
                dao.AuditLogDAO.log(null, "Admin", "Application " + decision, "Application",
        ...
        try {
            ...
            if (appIdStr != null && studentId != null) {
                ...
                if (app != null && app.getStudentID().equals(studentId)) {
                    // Only allow withdrawal if not already awarded/rejected
                    if (app.getStatus().equals(StatusValidator.APP_AWARDED)
                            || app.getStatus().equals(StatusValidator.APP_REJECTED)) {
                    ...
                    if (applicationDAO.updateStatus(appId, StatusValidator.APP_WITHDRAWN)) {
                        ...
                        // AUDIT LOG
                        dao.AuditLogDAO.log(null, studentId, "Application Withdrawn", "Application",
    ...
    private String formatApplicationsJson(List<Application> apps) {
        ...
        try {
            ...
            for (Application a : apps) {
                ...
                List<model.Document> docs = a.getDocuments();
                for (int i = 0; i < docs.size(); i++) {
                    model.Document d = docs.get(i);
                ...
                if (eval != null) {
                    ...
                    List<model.EvaluationScore> scores = evaluationDAO
                            .findScoresByEvalId(eval.getEvalID());
                    for (int i = 0; i < scores.size(); i++) {
                        model.EvaluationScore s = scores.get(i);
    ...
    private String getApplicationJsonById(int appId) {
        ...
        dao.ScholarshipDAO sDAO = new dao.ScholarshipDAO();
        model.Scholarship sObj = sDAO.findById(a.getScholarshipID());
    ...
    private String formatApplicationJsonSingle(Application a, model.Scholarship sObj) {
        String reviewerInfo = "";
        if (a.getReviewerID() != null) {
            reviewerInfo = String.format(", \"reviewerId\": \"%s\", \"reviewerName\": \"%s\"",
                    JsonUtils.escape(a.getReviewerID()), JsonUtils.escape(a.getReviewerName()));
        ...
        if (sObj != null) {
            boolean first = true;
            for (model.Criterion c : sObj.getCriteria()) {
                if (!first)
                    criteriaJson.append(",");
                first = false;
        ...
        if (eval != null) {
            ...
            List<model.EvaluationScore> scores = evaluationDAO.findScoresByEvalId(eval.getEvalID());
            for (int i = 0; i < scores.size(); i++) {
                model.EvaluationScore sc = scores.get(i);
